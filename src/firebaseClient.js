...
// marks the entry "resolving" immediately (visible to every admin session
// via the live bugs subscription, not just this tab), runs any real
// registered data remedy for it, then asks Claude to verify/report and
// settles the entry on "fixed" or back to "open" with a note either way.
// Always resolves (never throws) with { ok, resolved?, note?, reason? } —
// reason "not_configured" means ANTHROPIC_API_KEY hasn't been set on the
// backend yet.
export async function resolveChangeLogEntry(bugId) {
  const functions = functionsByRole[activeRole];
  if (!functions) return { ok: false, reason: "not_configured" };
  try {
    const call = httpsCallable(functions, "resolveChangeLogEntry");
    const result = await call({ bugId });
    return result.data;
  } catch (e) {
    console.error("[resolveChangeLogEntry] callable failed", e);
    return { ok: false, reason: "error" };
  }
}
