              ...
              <GuidedStep {...stepProps(0)} lang={lang}>
                <div className="px-2 py-2.5 text-center" style={{ borderBottom: `2px solid ${C.marigoldDeep}`, background: C.paper }}>
                  ...
                  <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0" autoFocus
                    className="w-full text-center outline-none bg-transparent" style={{ color: "#1D4ED8", fontFamily: monoFont, fontSize: 26, fontWeight: 900 }} />
                </div>
              </GuidedStep>
              <div className="grid grid-cols-2" style={{ background: C.paper }}>
                ...
                <GuidedStep {...stepProps(2)} lang={lang}>
                  <div className="px-1.5 py-2 text-center h-full flex flex-col justify-center" style={{ background: C.paper }}>
                    <div className="text-xs font-black mb-1" style={{ color: "#1D4ED8" }}>{lang === "en" ? "Waiting charges ₹/hr *" : "वेटिंग चार्ज ₹/घं *"}</div>
  ...
  return (
    <div className="mt-3 space-y-2.5">
      ...
      <div className={`rounded-lg p-3 shadow-lg ${!clock.isOvertime ? "guided-step-active" : ""}`} style={{ background: C.metallicBrown }}>
        {trip.hours ? (
          <>
            {clock.isOvertime ? (
              ...
              <>
                ...
                <div className="text-xl font-bold" style={{ color: "#000000", fontFamily: monoFont }}>{clock.remainingStr}</div>
              </>
            )}
          ...
          <>
            ...
            <div className="text-xl font-bold" style={{ color: "#000000", fontFamily: monoFont }}>{clock.elapsedStr}</div>
      ...
      <div className={`rounded-lg p-3 shadow-lg ${clock.isOvertime ? "guided-step-active" : ""}`} style={{ background: C.metallicBrown }}>
        ...
        <div className="text-xl font-bold" style={{ color: "#000000", fontFamily: monoFont }}>{clock.isOvertime ? clock.waitingElapsedStr : "00:00:00"}</div>
        {clock.isOvertime ? (
          trip.extraHourRate ? (
      ...
      <div className="flex-1 overflow-y-auto relative">
        ...
        {tab === "home" && rideView === "advance" && (
          selectedAdvanceId && advanceBookings.find((ab) => ab.id === selectedAdvanceId) ? (() => {
            ...
            <div className="px-5 py-4">
              <button onClick={() => setRideView("current")} className="flex items-center gap-1 mb-4 pl-2 pr-3 py-1.5 rounded-full text-sm font-black shadow-sm" style={{ background: C.marigold, color: C.navy, border: `1.5px solid ${C.marigoldDeep}` }}>
                <ChevronLeft size={18} strokeWidth={3} /> {lang === "en" ? "Back" : "वापस"}
              </button>
              {advanceBookings.length === 0 ? (
...
function TermsModal({ open, onClose, commissionPct, bonusPct, lang }) {
  ...
  const sections = lang === "en" ? [
    ...
    ["9. Overload Charges", "Apna Transport is not responsible for any overload fine, challan, or penalty charged by the RTO, traffic police, or any other authority to either the customer or the driver. Ensuring the vehicle is loaded within its permitted capacity is the joint responsibility of the customer (who declares the weight) and the driver/vehicle owner (who accepts the load) — any overload-related charge or legal action must be settled directly between them."],
  ] : [
